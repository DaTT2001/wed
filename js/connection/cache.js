...
export const cache = (cacheName) => {
    ...
    const get = (input, cancel = null) => {
         ...
         */
        const fetchPut = () => {
            // Resolve relative URLs against the current page to avoid sending local asset requests to the API base URL
            let target = input;

            try {
                // If input is an absolute URL this will succeed
                const u = new URL(String(input));
                target = u.toString();
            } catch (e) {
                // Relative URL — resolve against current page
                target = new URL(String(input), window.location.href).toString();
            }

            return request(HTTP_GET, target).withCancel(cancel).withRetry().default();
        };
