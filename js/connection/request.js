            ...
            return baseFetch(new URL(path, document.body.getAttribute('data-url'))).then((res) => {
                ...
                return res.json().then((json) => {
                    if (json.error) {
                        ...
                        const e = new Error(isErrServer ? `ID: ${json.id}\n🟥 ${msg}` : `🟨 ${msg}`);
                ...
                console.error('Request error', { method: req.method, path, status: err.status ?? null }, err);
                ...
                // detect local dev hosts to avoid bothering with external rate-limit popups
                const isLocalHost = (['localhost', '127.0.0.1'].includes(window.location.hostname) || window.location.hostname.startsWith('192.168.') || window.location.hostname.startsWith('10.'));

                const message = `${err.message ?? String(err)}\nRequest: ${req.method} ${path}${err.status ? ` (status ${err.status})` : ''}`;

                if (err.status === 429 && isLocalHost) {
                    // suppress alert during local development and log a warning instead
                    console.warn('Suppressed 429 alert on local host:', message);
                } else {
                    alert(message);
                }
