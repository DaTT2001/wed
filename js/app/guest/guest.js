...
export const guest = (() => {
    ...
    const domLoaded = () => {
        ...
        if (token && token.length > 0) {
            ...
            // If running in a local/dev environment, skip calling the remote API (avoids rate-limit popups)
            ...
            if (isLocalHost) {
                // Local fallback: proceed to load media/libs and complete config/comment progress locally
                vid.load();
                aud.load();
                lib.load({ confetti: document.body.getAttribute('data-confetti') === 'true' });

                // mark config as completed with a sensible default
                try {
                    const cfg = storage('config');
                    cfg.set('tz', Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC');
                } catch (e) {
                    /* ignore */
                }
